...
export default function StudentDashboard() {
  ...
  const avgProgress = valid.length ? Math.round(valid.reduce((s, e) => s + e.progress, 0) / valid.length) : 0;
  ...
  /* ── stat cards ── */
  const STATS = [
    { label: 'Tugatilgan kurslar', value: String(completed.length), Icon: CheckCircle2, grad: 'linear-gradient(135deg,#22C55E,#06B6D4)', shadow: 'rgba(34,197,94,0.32)', pct: `${Math.min(100, completed.length * 20)}%`, trend: '+1', TrendIcon: TrendingUp, trendColor: '#22C55E' },
    { label: 'Faol kurslar',       value: String(inProgress.length), Icon: BookOpen,     grad: 'linear-gradient(135deg,#4F46E5,#7C3AED)', shadow: 'rgba(79,70,229,0.32)',  pct: `${Math.min(100, inProgress.length * 20)}%`, trend: '+2', TrendIcon: TrendingUp, trendColor: '#22C55E' },
    ...
    { label: 'O\'qish foizi',      value: `${avgProgress}%`,        Icon: Target,       grad: 'linear-gradient(135deg,#06B6D4,#4F46E5)', shadow: 'rgba(6,182,212,0.30)',  pct: `${avgProgress}%`, trend: '+6%', TrendIcon: TrendingUp, trendColor: '#22C55E' },
  ];
  ...
  const s = (base) => ({ ...base, background: 'var(--surface)', color: 'var(--text)', borderColor: 'var(--border)' });

  if (loading) {
    return (
      <div data-theme={theme} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: 'var(--bg)' }}>
  ...
  return (
    <div data-theme={theme} style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif", color: 'var(--text)', WebkitFontSmoothing: 'antialiased' }}>
      <style>{THEME_VARS}</style>
      ...
      <aside className="ds-sidebar"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '4px 8px 0' }}>
          ...
          <button className="flex md:hidden" onClick={() => setSidebarOpen(false)} style={{ marginLeft: 'auto', width: 30, height: 30, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface-2)', cursor: 'pointer', color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={16} />
          </button>
        </div>
        ...
        <nav className="ds-scroll" style={{ flex: 1, marginTop: 26, display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
          {MENU.map((m) => {
            const active = location.pathname === m.to && m.to === '/student' || (m.to !== '/student' && location.pathname.startsWith(m.to));
            return (
              <Link key={m.label} to={m.to} onClick={() => setSidebarOpen(false)}
                ...
                <span>{m.label}</span>
                {m.badge > 0 && (
                  <span style={{ marginLeft: 'auto', minWidth: 20, height: 20, padding: '0 6px', borderRadius: 99, background: active ? '#4F46E5' : (m.label === 'Xabarlar' ? '#EF4444' : 'var(--surface-3)'), color: (active || m.label === 'Xabarlar') ? '#fff' : 'var(--text-soft)', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {m.badge}
                  </span>
                )}
              </Link>
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '11px 13px', borderRadius: 12, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 14.5, color: '#EF4444', width: '100%', textAlign: 'left', fontFamily: 'inherit' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }} className="ds-main-wrap">
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 40, backdropFilter: 'saturate(180%) blur(16px)', WebkitBackdropFilter: 'saturate(180%) blur(16px)', background: isDark ? 'rgba(20,29,49,0.88)' : 'rgba(255,255,255,0.88)', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 24px' }}>
            <button className="flex md:hidden" onClick={() => setSidebarOpen(true)}
              style={{ alignItems: 'center', justifyContent: 'center', width: 42, height: 42, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', color: 'var(--text)', display: 'flex' }}>
              <Menu size={20} />
            </button>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '5px 8px 5px 5px', borderRadius: 14, border: '1px solid var(--border)', background: 'var(--surface)' }}>
                <span style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#4F46E5,#7C3AED)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 14 }}>{initials}</span>
        ...
        {/* CONTENT */}
        <div className="ds-scroll" style={{ flex: 1, padding: '26px 24px 30px', display: 'flex', flexDirection: 'column', gap: 24, animation: 'fadeUp .5s ease both', overflowY: 'auto' }}>

          {/* WELCOME HERO */}
          <section style={{ position: 'relative', overflow: 'hidden', borderRadius: 24, padding: 30, background: 'radial-gradient(500px 280px at 12% 10%,rgba(6,182,212,0.45),transparent 60%),radial-gradient(560px 360px at 90% 100%,rgba(124,58,237,0.55),transparent 60%),linear-gradient(135deg,#4F46E5,#7C3AED)', boxShadow: '0 20px 44px rgba(79,70,229,0.32)' }}>
            <div style={{ position: 'absolute', width: 240, height: 240, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.16)', top: -90, right: -50 }} />
            <div style={{ position: 'relative', display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ maxWidth: 520 }}>
                <h1 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 'clamp(22px,3vw,34px)', letterSpacing: '-0.02em', color: '#fff', margin: 0 }}>Salom, {user?.name?.split(' ')[0]} 👋</h1>
                <p style={{ fontSize: 16, lineHeight: 1.55, color: 'rgba(255,255,255,0.9)', margin: '10px 0 0' }}>
                  {inProgress.length > 0 ? `Sizni ${inProgress.length} ta kurs davom etishini kutmoqda.` : "Bugun ham yangi bilimlarni o'rganishni boshlang!"}
                </p>
                <Link to="/student/courses" style={{ marginTop: 20, display: 'inline-flex', alignItems: 'center', gap: 9, padding: '12px 22px', borderRadius: 13, background: '#fff', color: '#4F46E5', fontFamily: "'Plus Jakarta Sans'", fontWeight: 700, fontSize: 15, textDecoration: 'none', boxShadow: '0 12px 26px rgba(0,0,0,0.18)', transition: 'transform .2s' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '18px 22px', borderRadius: 20, background: 'rgba(255,255,255,0.14)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.22)' }}>
                <div style={{ position: 'relative', width: 88, height: 88, borderRadius: '50%', background: `conic-gradient(#fff ${Math.round(avgProgress * 3.6)}deg, rgba(255,255,255,0.25) 0)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(79,70,229,0.55)', backdropFilter: 'blur(4px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                ...
                <div style={{ color: '#fff' }}>
                  ...
                  <div style={{ fontSize: 12.5, opacity: 0.8, marginTop: 4 }}>{completed.length > 0 ? 'Ajoyib ish! Davom eting 🔥' : 'Boshlang, muvaffaqiyat siz bilan!'}</div>
                </div>
              </div>
            </div>
          </section>
          ...
          {/* STAT CARDS */}
          <section className="ds-stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 18 }}>
            {STATS.map((s) => (
              <div key={s.label} style={{ borderRadius: 20, padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', transition: 'transform .2s' }}
                onMouseEnter={(e) => (e.currentTarget.style.transform = 'translateY(-3px)')}
                onMouseLeave={(e) => (e.currentTarget.style.transform = 'none')}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ width: 46, height: 46, borderRadius: 13, background: s.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', boxShadow: `0 8px 18px ${s.shadow}` }}><s.Icon size={22} /></span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 12.5, fontWeight: 700, color: s.trendColor }}><s.TrendIcon size={13} />{s.trend}</span>
                </div>
          ...
          {continueCourses.length > 0 && (
            <section>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <Link to="/student/courses" style={{ fontSize: 14, fontWeight: 700, color: '#4F46E5', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}>Barchasi <ArrowRight size={15} /></Link>
              </div>
              <div className="ds-cont-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 18 }}>
                {continueCourses.map((c) => (
                  <div key={c._id} style={{ borderRadius: 20, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', overflow: 'hidden', transition: 'transform .2s' }}
                    ...
                    <div style={{ position: 'relative', height: 108, background: c.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      ...
                      <span style={{ position: 'relative', width: 54, height: 54, borderRadius: 15, background: 'rgba(255,255,255,0.22)', backdropFilter: 'blur(6px)', border: '1px solid rgba(255,255,255,0.32)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}><c.Icon size={25} /></span>
                    ...
                    <div style={{ padding: 18 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: 'var(--text-soft)', marginTop: 6 }}><User size={14} />{c.mentor}</div>
                      ...
                      <Link to={`/student/courses/${c._id}`} style={{ width: '100%', marginTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 11, borderRadius: 12, background: 'var(--surface-2)', color: 'var(--text)', fontFamily: "'Plus Jakarta Sans'", fontWeight: 700, fontSize: 14, textDecoration: 'none', transition: 'background .2s,color .2s' }}
          ...
          {/* MAIN GRID */}
          <div className="ds-main-grid" style={{ display: 'grid', gridTemplateColumns: '1.7fr 1fr', gap: 24, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, minWidth: 0 }}>
              ...
              <section style={{ borderRadius: 20, padding: 24, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14, height: 190, marginTop: 24, padding: '0 4px' }}>
                  {WEEK_CHART.map((d, i) => (
                    <div key={d.day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9, height: '100%', justifyContent: 'flex-end' }}>
                      <div style={{ width: '100%', maxWidth: 34, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 3, height: '100%' }}>
                        <div style={{ height: `${d.b}%`, borderRadius: '6px 6px 0 0', background: '#06B6D4', animation: 'growBar .9s ease both' }} />
                        <div style={{ height: `${d.a}%`, borderRadius: d.b > 0 ? 0 : '6px 6px 0 0', background: 'linear-gradient(180deg,#7C3AED,#4F46E5)', animation: 'growBar .9s ease both' }} />
                      </div>
              ...
              <section style={{ borderRadius: 20, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
                ...
                <div style={{ overflowX: 'auto' }} className="ds-scroll">
                  {assignRows.length > 0 ? (
                      ...
                      <thead>
                        <tr>
                          {['Topshiriq', 'Kurs', 'Muddat', 'Holati', 'Amal'].map((col) => (
                            <th key={col} style={{ padding: '11px 22px', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.03em', color: 'var(--text-mute)', background: 'var(--surface-2)', textAlign: 'left' }}>{col}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {assignRows.map((a, i) => (
                          <tr key={i} style={{ borderTop: '1px solid var(--border)' }}>
                            <td style={{ padding: '15px 22px', fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>{a.task}</td>
                            <td style={{ padding: '15px 22px', fontSize: 13.5, color: 'var(--text-soft)' }}>{a.course}</td>
                            <td style={{ padding: '15px 22px', fontSize: 13.5, color: 'var(--text-soft)' }}>{a.due}</td>
                            <td style={{ padding: '15px 22px' }}>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px', borderRadius: 99, fontSize: 12, fontWeight: 700, background: a.bg, color: a.color }}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: a.color }} />{a.status}
                              </span>
                            </td>
                            <td style={{ padding: '15px 22px' }}>
                              <Link to="/student/assignments" style={{ fontSize: 13.5, fontWeight: 700, color: '#4F46E5', textDecoration: 'none' }}>{a.action}</Link>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <div style={{ padding: '40px 22px', textAlign: 'center', color: 'var(--text-soft)', fontSize: 14 }}>Hozircha topshiriqlar yo'q</div>
                  )}
                </div>
              </section>
                    ...
                    {certs.map((c, i) => (
                      <div key={i} style={{ borderRadius: 18, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: 16 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: 'var(--text-soft)', marginTop: 5 }}><CalendarCheck size={14} />{c.date}</div>
                          <button style={{ width: '100%', marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: 10, border: '1px solid var(--border)', cursor: 'pointer', borderRadius: 11, background: 'var(--surface-2)', color: 'var(--text)', fontFamily: "'Plus Jakarta Sans'", fontWeight: 700, fontSize: 13.5, transition: 'background .2s,color .2s' }}
              ...
              <section style={{ borderRadius: 20, padding: 24, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 14 }}>
                  {ACHIEVEMENTS_RAW.map((b) => (
                    <div key={b.label} style={{ textAlign: 'center', padding: '18px 12px', borderRadius: 16, background: b.unlocked ? 'var(--surface-2)' : 'transparent', border: '1px solid var(--border)', opacity: b.unlocked ? 1 : 0.6 }}>
                      <div style={{ width: 52, height: 52, margin: '0 auto', borderRadius: '50%', background: b.unlocked ? b.grad : 'var(--surface-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: b.unlocked ? '#fff' : 'var(--text-mute)', boxShadow: b.unlocked ? '0 8px 18px rgba(79,70,229,0.25)' : 'none' }}>
                        {b.unlocked ? <b.Icon size={24} /> : <Lock size={24} />}
                      </div>
                      <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text)', marginTop: 11, lineHeight: 1.3 }}>{b.label}</div>
            ...
            <div className="ds-right-col" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <section style={{ borderRadius: 20, padding: 24, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: 18, paddingTop: 18, borderTop: '1px solid var(--border)' }}>
                  {[
                    [String(valid.length), 'Kurslar'],
                    [String(completed.length), 'Tugatilgan'],
                    ['2026', 'Boshlangan'],
                  ].map(([n, l]) => (
                    <div key={l}>
                  ...
                  {UPCOMING.map((l) => (
                    <div key={l.title} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 13, borderRadius: 14, background: 'var(--surface-2)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.title}</div>
                      ...
                      <button title="Qo'shilish" style={{ flexShrink: 0, width: 36, height: 36, borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#4F46E5,#7C3AED)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform .2s' }}
              ...
              <section style={{ borderRadius: 20, padding: 22, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[ChevronLeft, ChevronRight].map((Icon, i) => (
                      <button key={i} style={{ width: 30, height: 30, borderRadius: 9, border: '1px solid var(--border)', background: 'var(--surface-2)', cursor: 'pointer', color: 'var(--text-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon size={16} /></button>
                    ))}
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 5 }}>
                  ...
                  {calendar.map((c, i) => (
                    <div key={i} style={{ aspectRatio: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 9, fontSize: 12.5, fontWeight: 600, color: c ? c.color : 'transparent', background: c ? c.bg : 'transparent', border: c ? c.border : 'none' }}>
                      {c ? c.d : ''}
                    </div>
                  ))}
                </div>
              ...
              <section style={{ borderRadius: 20, padding: 22, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {NOTIFS.map((n, i) => (
                    <div key={i} style={{ display: 'flex', gap: 12, padding: 11, borderRadius: 13, transition: 'background .2s', cursor: 'pointer' }}
                      onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--surface-2)')}
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
                      <span style={{ flexShrink: 0, width: 38, height: 38, borderRadius: 11, background: n.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: n.color }}><n.Icon size={18} /></span>
                      <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {/* MOBILE BOTTOM NAV */}
      <nav className="ds-bottom-nav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 70, display: 'none', alignItems: 'center', justifyContent: 'space-around', padding: '9px 8px', background: isDark ? 'rgba(20,29,49,0.92)' : 'rgba(255,255,255,0.92)', backdropFilter: 'blur(16px)', borderTop: '1px solid var(--border)' }}>
        {BOTTOM_NAV.map((b) => {
          const active = location.pathname === b.to;
          return (
            <Link key={b.label} to={b.to} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: 6, textDecoration: 'none', color: active ? '#4F46E5' : 'var(--text-mute)', fontSize: 11, fontWeight: 700 }}>
...
function IconBtn({ children, onClick, title, style = {} }) {
  return (
    <button onClick={onClick} title={title}
      style={{ width: 42, height: 42, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background .2s', position: 'relative', ...style }}
      onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--surface-2)')}
